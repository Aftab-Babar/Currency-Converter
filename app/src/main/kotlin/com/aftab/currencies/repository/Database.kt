...
import com.aftab.currencies.util.*
...
class Database(context: Context) {
    ...
    fun setApiProvider(api: com.aftab.currencies.model.ApiProvider) {
        prefs.apply {
            edit().putInt(keyApi, api.number).apply()
        }
    }

    private val defaultProvider = com.aftab.currencies.model.ApiProvider.EXCHANGERATE_HOST

    fun getApiProvider(): com.aftab.currencies.model.ApiProvider {
        val provider = com.aftab.currencies.model.ApiProvider.fromNumber(prefs.getInt(keyApi, defaultProvider.number))
        if (provider == null)
            setApiProvider(defaultProvider)
        return provider ?: defaultProvider
    }

    fun getApiProviderAsync(): LiveData<com.aftab.currencies.model.ApiProvider> {
        return SharedPreferenceIntLiveData(prefs, keyApi, defaultProvider.number).map {
            val provider = com.aftab.currencies.model.ApiProvider.fromNumber(it)
            if (provider == null)
                setApiProvider(defaultProvider)
